...
```{r}
#| fig-align: center
plotlyOutput("vowel_plot")
```

</div>
:::

Let's focus in on some specific areas
...
output$vowel_plot <- renderPlotly({
  ...
  get_data() |> 
    ggplot(aes(-F2, -F1, label = ipa_vclass, color = ipa_vclass, text = word))+
      geom_text() -> p
  ply <- ggplotly(p, tooltip = "text", width = 600, height = 500)
  return(ply)
})
```
...
output$lowback <- renderPlotly({
  ...
  lowback |> 
    ggplot(aes(-F2, -F1))+
      geom_text(aes(label = ipa_vclass, text = word, color = ipa_vclass))->p
